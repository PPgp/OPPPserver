...
#' @details The function uses the \code{\link[wpp2024]{UNlocations}} dataset of the
#'      \pkg{wpp2024} package. By default it selects locations where \code{location_type} is 4.
...
#' @details The function uses the \code{\link[wpp2024]{UNlocations}} dataset of the
#'      \pkg{wpp2024} package. It selects locations where \code{location_type} is other than four,
...
#'      If \code{n} is 5, the dataset is aggregated into 5-year age groups.
#' @export
#'
#' @examples
#' # Retrieve and plot population of Spain for 2024 and 1960
#' spain_pop <- get_wpp_pop("Spain") # default of 2024
#' plot(spain_pop[, age], spain_pop[, popF], type = "l", col = "red",
#'     main = "Population of Spain in 2024 and 1960",
...
#'
get_wpp_pop <- function(country, year = 2024, n = 1){
    ...
    if(n == 5){
        ...
        pop_res <- pop_res[, .(popM = sum(popM), popF = sum(popF)), by = c("agecat", "age5")][, agecat := NULL]
    }
    return(pop_res)
}
...
#'
#' @details The data is extracted by combining datasets \code{\link[wpp2024]{tfr1dt}} and
#'      \code{\link[wpp2024]{tfrproj1dt}}.
#' @export
#'
#' @examples
...
#'
#' @details The data is extracted by combining datasets \code{\link[wpp2024]{e01dt}} and
#'      \code{\link[wpp2024]{e0proj1dt}}.
#' @export
#'
#' @examples
...
#'
#' @details The data corresponds to the dataset \code{\link[wpp2024]{migration1dt}}.
#' @export
#'
#' @examples
...
#' @keywords internal
#' @export
get_wpp <- memoise::memoise(function(dataset_name, package = "wpp2024"){
